...
if(!$conexao){
    ...
    if(empty($requestData['nome']) && empty($requestData['ativo'])){
        ...
        VALUES('$requestData[nome]', '$requestData[email]', '$requestData[telefone]', '$requestData[ativo]', '$requestData[dataagora]', '$requestData[dataagora]')";
        ...
        if($resultado){
            ...
            $dados = array(
                'tipo' => 'error',
                'mensagem' => 'Não foi possível cadastrar o cliente.'.mysqli_error($conexao)               
            );
        }      
    }
